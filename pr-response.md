...
**Engagement with reviewer's point:** The maintainer's argument is that _most users want to see what they added recently_, and for a watchlist I think that's exactly right — so I'm agreeing, but with evidence rather than just deferring. A watchlist is a queue of intent: you add a film the moment a friend recommends it or a trailer lands, and the next time you open CineLog to pick something for tonight, that just-queued film is the one you're most likely reaching for. Newest-first puts it where your eye lands; alphabetical actively fights it, burying tonight's impulse under every "A" title you saved months ago and forgot. Alphabetical only wins when you already know the exact title and are scanning a long list to find it — but a personal watchlist is usually short enough to skim, and you rarely arrive knowing the title. The one case the maintainer's rule undersells is a long-neglected backlog, where oldest-first could help you finally clear it — but that's a power-user need better met by an explicit sort option than by overriding a sensible default.
